...
from vibrant.orchestrator.types import ReviewResolutionRecord, ReviewTicket, ReviewTicketStatus, utc_now
...
class ReviewTicketStore:
    ...
    def resolve(
        ...
        *,
        ...
        if resolved_status is None and resolution is not None:
            resolved_status = _decision_to_status(resolution.decision)
        if resolved_status is None:
            raise ValueError("resolve() requires either a resolution record or an explicit status")
...
def _decision_to_status(decision: str) -> ReviewTicketStatus:
    return {
