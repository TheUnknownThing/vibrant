...
class ResourceHandlers:
    ...
    def role_list(self) -> list[dict[str, Any]]:
        return [_serialize_value(role) for role in self.facade.roles.list()]
    ...
    def task_instances(self, *, task_id: str | None = None, agent_id: str | None = None) -> dict[str, Any]:
        ...
        instances = self.facade.instances.list(task_id=resolved_task_id, include_completed=True)
        latest_instance = instances[-1] if instances else None
        return {
            "task": _serialize_task(task),
            "instances": [_serialize_value(snapshot) for snapshot in instances],
            "latest_instance": _serialize_value(latest_instance),
        }

    def instance_by_id(self, agent_id: str) -> dict[str, Any]:
        snapshot = self.facade.instances.get(agent_id)
        if snapshot is None:
...
def _resolve_task_id(facade: OrchestratorFacade, *, task_id: str | None, agent_id: str | None) -> str:
    ...
    snapshot = facade.instances.get(agent_id)
    if snapshot is None:
        raise KeyError(f"Unknown agent instance: {agent_id}")
