...
class RoadmapStore:
    ...
    def replace_task(
        ...
        task: TaskInfo,
        *,
        active_attempt_id: str | None | object = _UNSET,
    ) -> TaskInfo:
        document = self.load()
        for index, current in enumerate(document.tasks):
            if current.id != task.id:
                continue
            document.tasks[index] = task.model_copy(deep=True)
            break
        else:
        ...
        if active_attempt_id is not _UNSET:
            meta = self._load_meta()
            entry = meta.setdefault(task.id, {"definition_version": 1, "active_attempt_id": None})
            entry["active_attempt_id"] = active_attempt_id
            self._save_meta(meta)
        return _require_task(self.load(), task.id)
