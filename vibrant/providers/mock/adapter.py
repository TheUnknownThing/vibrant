...
class MockCodexAdapter(ProviderAdapter):
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        self.provider_thread_id = kwargs.get("resume_thread_id")
    ...
    async def start_session(self, *, cwd: str | None = None, **kwargs: Any) -> Any:
        return {"serverInfo": {"name": "mock-codex"}, "cwd": cwd or str(self.cwd), **kwargs}
    ...
    async def start_thread(self, **kwargs: Any) -> Any:
        ...
        return {"thread": {"id": self.provider_thread_id, "path": self.thread_path}, **kwargs}
    ...
    async def resume_thread(self, provider_thread_id: str, **kwargs: Any) -> Any:
        ...
        return {"thread": {"id": provider_thread_id, "path": self.thread_path}, **kwargs}
    ...
    async def _run_turn(
        ...
        *,
        ...
        for chunk in _chunk_text(response, chunk_size=18 if scenario.long_response else 24):
            await self._emit({"type": "content.delta", "turn_id": turn_id, "delta": chunk})
    ...
    async def _emit_tool_call(self, turn_id: str) -> None:
        request_id = f"mock-tool-{uuid4().hex[:6]}"
