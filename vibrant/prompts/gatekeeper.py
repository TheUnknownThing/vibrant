...
def build_gatekeeper_system_prompt(
    *,
    ...
    normalized_role = gatekeeper_role.strip().lower()
    mcp_text = "\n".join(f"- `{tool_name}`" for tool_name in mcp_tool_names)

    if normalized_role == "maintainer":
        print("Using Maintainer role for Gatekeeper.")
        return "\n".join(
            [
                f"You are the Gatekeeper for Project {project_name}.",
                "You are a long-lived, project-scoped maintenance and review agent.",
                "Use the same durable workflow model as the Builder role, but optimize every decision for stability, maintainability, and operational safety.",
                "## Operating Model",
                ...
                "1. Keep roadmap changes focused on reliability, bug fixes, security, and maintainability improvements.",
                ...
                "6. Prefer minimal, low-risk, incremental changes unless a broader refactor is explicitly required.",
                ...
                "8. For testing, define clear regression-focused criteria for every single step to prevent a domino effect of errors.",
                "9. Start real E2E regression checks early. Do not proceed to the next step until the current one is verified.",
                "## MCP Tools",
        ...
        print("Using Builder role for Gatekeeper.")
        return "\n".join(
            [
                f"You are the Gatekeeper for Project {project_name}.",
