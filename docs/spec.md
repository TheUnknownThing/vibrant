...
## 7. TUI Specification
### 7.1 Framework
- **Library**: [Textual](https://github.com/Textualize/textual) (built on Rich)
- **Reason**: Full-featured TUI framework with widget system, CSS-like styling, reactive data binding, and async support.
### 7.2 Layout
The TUI is a 4-panel layout:
```
┌─────────────────┬─────────────────────────────────┐
│                 │                                 │
│   (A) Plan      │   (B) Agent Output Streams      │
│   Task Tree     │   (real-time, switchable)       │
│                 │                                 │
│   ▸ task-001 ✓  │   [agent-task-003] Running...   │
│   ▸ task-002 ✓  │   > Installing dependencies...  │
│   ▸ task-003 ⟳  │   > Running tests...            │
│   ▸ task-004 ○  │   > Modified src/app.py         │
│   ▸ task-005 ○  │                                 │
│                 │                                 │
├─────────────────┼─────────────────────────────────┤
│                 │                                 │
│   (C) Consensus │   (D) Chat / Q&A                │
│   Pool View     │                                 │
│                 │   [Gatekeeper → User]            │
│   Status: EXEC  │   Q: Should auth use OAuth       │
│   Version: 14   │      or API keys?                │
│   Tasks: 3/8    │                                 │
│   Decisions: 5  │   > Type your answer...          │
│                 │                                 │
└─────────────────┴─────────────────────────────────┘
 [F1:Help] [F2:Pause] [F3:Consensus] [F5:Switch Agent] [F10:Quit]
```
### 7.3 Panel Details
#### Panel A — Plan / Task Tree
- Displays all tasks from `roadmap.md` as a tree view
- Selecting a task shows its details (acceptance criteria, prompt, agent summary) in an overlay
- Color-coded by priority: critical=red, high=orange, medium=yellow, low=default
#### Panel B — Agent Output Streams
- Real-time streaming of the active agent's canonical provider events and assistant text deltas
- Raw stderr / native event logs are available as a secondary debug view
- Tabs or key-switchable between active agents (e.g., `F5` cycles through)
- Ring buffer of last 10,000 lines per agent
- Auto-follows latest output (toggle scroll lock with `S`)
#### Panel C — Consensus Pool View
- Summary view of the consensus pool:
  - Current project status
  - Consensus version number
  - Task progress (completed/total)
  - Recent decisions (last 3)
  - Pending questions count (highlighted if > 0)
- Pressing `F3` opens full consensus Markdown in a scrollable view
#### Panel D — Chat / Q&A Panel
- Displays the active conversation:
  - During **planning**: User ↔ Gatekeeper dialogue
  - During **execution**: Gatekeeper questions escalated to User
- User can type responses in an input field at the bottom
- Switchable between conversation threads (Gatekeeper sessions, previous agent conversations)
- History is scrollable
### 7.4 Notifications
- When the Gatekeeper escalates a question to the user:
  - Panel D flashes/highlights with the new question
  - A banner appears at the top: `⚠ Gatekeeper needs your input — see Chat panel`
  - Terminal bell is triggered (configurable)
---
## 8. Agent Integration — OpenAI Codex CLI
Vibrant should integrate with Codex through `codex app-server` rather than relying on one-shot CLI invocations plus heuristic stdout parsing.
### 8.1 Integration Model
- Each Vibrant agent owns a dedicated `codex app-server` child process.
- The Orchestrator communicates with Codex using newline-delimited JSON-RPC messages over stdin/stdout. This is JSON-RPC over stdio, but it is **not** LSP-style `Content-Length` framing.
- The Orchestrator treats Codex as a provider behind a provider-neutral internal interface.
- Each Vibrant `agent_id` maps to a durable Codex provider thread identified primarily by `provider_thread_id`. Vibrant may additionally persist its own replay/log cursor metadata, but resume must not depend on Codex returning a dedicated server-side `resume_cursor` field.
- The TUI, validation pipeline, and recovery logic consume normalized runtime events, not raw terminal output as the primary source of truth.
### 8.2 Session Lifecycle & Spawning
Agents are spawned as child processes of the Orchestrator in app-server mode:
```python
...
```
After spawn, the Orchestrator performs a structured session handshake:
1. Send `initialize` over JSON-RPC.
2. Advertise `capabilities.experimentalApi = true`, because Vibrant relies on the modern `thread/*` and `turn/*` APIs and on `persistExtendedHistory` support.
3. Send the `initialized` notification after the `initialize` response is received.
4. Open a provider thread with `thread/start` or resume it with `thread/resume` when a stored `provider_thread_id` exists.
5. Persist the returned thread metadata, especially `thread.id` and `thread.path` / rollout-path information, into the agent record.
6. Emit session lifecycle events into the TUI and durable state store.
7. Start one or more turns for the assignment with `turn/start`; interrupt in-flight turns with `turn/interrupt` when needed.

**Runtime modes**
- Vibrant's internal runtime modes are `read_only`, `workspace_write`, and `full_access`.
- These map to Codex thread sandbox modes `read-only`, `workspace-write`, and `danger-full-access` respectively.
- `turn/start` does **not** use those string enums directly. It expects a `sandboxPolicy` object whose type variants are camelCase, such as `readOnly`, `workspaceWrite`, and `dangerFullAccess`.
- Approval is configured separately through `approvalPolicy`. For autonomous v1 agents, the default should be `never` unless a future workflow explicitly opts into operator approval.

**Provider options**
- The Codex binary path, launch args, `CODEX_HOME`, model, model-provider, approval policy, reasoning-effort, reasoning-summary, thread sandbox mode, turn sandbox policy, and extra config overrides should be configurable via `vibrant.toml`.
- Model selection and reasoning settings should be passed as structured session/turn options, not encoded into prompt text.
### 8.3 Turn Execution & Prompt Construction
Each assignment is generally executed as a single Codex turn within the session. The prompt remains intentionally task-scoped:
```
You are a code agent working on Project {name}.
## Your Task
{task.title}
## Acceptance Criteria
{task.acceptance_criteria, formatted as checklist}
## Context
{relevant project files, injected}
## Skills
{loaded skill file contents}
## Rules
1. You are working in a git worktree on branch `{task.branch}`.
...
4. Commit your changes with a descriptive message prefixed with `[vibrant:{task.id}]`.
```
The prompt is sent as structured turn input rather than as a positional shell argument. In practice, Vibrant should call `turn/start` with an `input` array. The minimal text input item is:
```json
{"type": "text", "text": "...prompt...", "text_elements": []}
```
Attachments and future multimodal context should also be delivered via structured provider payloads when needed.
